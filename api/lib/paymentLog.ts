/**
 * Payment logging: console (always) + Supabase (if configured).
 * Use for reconciliation with Telegram payouts.
 */
...
export async function logPayment(record: PaymentRecord): Promise<void> {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error('Payment log insert failed', res.status, t)
    }
  } catch (e) {
...
export async function updatePaymentResult(
  ...
  try {
    const res = await fetch(
      `${url}/rest/v1/payments?telegram_payment_charge_id=eq.${encodeURIComponent(telegramPaymentChargeId)}`,
      {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
